...
def export_schedule_excel(schedule_df: pd.DataFrame, year: int, month: int) -> bytes:
    ...
    for column_index, item in enumerate(days, start=3):
        ws.cell(row=2, column=column_index, value=item["label"])
        ws.cell(row=3, column=column_index, value=item["weekday"])

    for row_index, record in enumerate(schedule_df.to_dict("records"), start=4):
    ...
    for column_index in range(1, max_column + 1):
        ws.cell(row=2, column=column_index).fill = HEADER_FILL
        ws.cell(row=3, column=column_index).fill = HEADER_FILL
